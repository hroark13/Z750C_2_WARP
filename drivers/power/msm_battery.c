 ...
 */
#define DEBUG  0  //shiyan modify from 0
...
//shiyan modify for v70 20120217 +++
...
static int  batt_cap_resume =0;
static int  batt_has_charged_full =0;
static int last_battery_capacity = 1000;
//#define CHG_FINAL_CYCLE_TIME 10       //doumingming  abandon final_time 20121221
#define TIMEOUT_MIN 432
//#define BATTERY_DEBUG 1
//extern int read_otg_flag(void);
//doumingming add 20121106 ++
static struct timespec chg_start;
static struct timespec chg_current;
static struct timespec unchg_start;
static struct timespec unchg_current;
static struct timespec unchg_sleep_time_start;
static struct timespec unchg_sleep_time_current;
//static int initial_battery_capacity;
static int charge_flag = 0; //0:initial  1:charging 2:uncharging
//doumingming add 20121106 --
#endif
//shiyan modify for v70 20120217 ---
...
static enum power_supply_property msm_batt_power_props[] = {
	...
	#ifdef CONFIG_ZTE_PLATFORM//shiyan added	
				       ...
				       union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_VOLTAGE_NOW:
		val->intval = msm_batt_info.battery_voltage*1000;   //shiyan added
		break;
...
static void msm_batt_update_psy_status(void)
{
	...
	#ifdef BATTERY_DEBUG
	printk("============================\n");
	printk("shiyan: enter msm_batt_update_psy_status\n");
	#endif
	pr_debug("%s: enter\n", __func__);	   
	if (msm_batt_get_batt_chg_status())
	{
		printk("error chg_debug is here return\n");
		return;
	}
...
//doumingming add 20121212 add for v72a temp protect warning ++
       #ifdef CONFIG_PROJECT_V72A
	battery_temp = 37;      //doumingming 20121211
	#else
...
//doumingming add 20121212 add for v72a temp protect warning --
		    ...
		    charger_type == CHARGER_TYPE_USB_WALL) 
	{
		...
		{			
			...
			#ifdef BATTERY_DEBUG
			printk("shiyan: usb charger_wake_lock lock.\n");	
			#endif
...
/*ZTE BSP doumingming 20120924 modify for give enough time to topper's processing START */
			wake_lock_timeout(&charger_wake_lock, 3000);
/*ZTE BSP doumingming 20120924 modify for give enough time to topper's processing END */
			#ifdef BATTERY_DEBUG
			printk("shiyan: usb charger_wake_lock unlock.\n");		
			#endif
	...
	//if(!read_otg_flag())
	{
		if (msm_batt_info.charger_type != charger_type) 
		{
			...
			{
				printk("BATT: USB charger plugged in\n");
				msm_batt_info.current_chg_source = USB_CHG;
				supp = &msm_psy_usb;
			} 
			else if (charger_type == CHARGER_TYPE_WALL) 
			{
				printk("BATT: AC Wall changer plugged in\n");
	...
	//}
	//[ECID:0000]ZTE_BSP maxiaoping 20120830 force to update charger status when power supply changed,start.
	if (supp) 
	{	
		//DBG_LIMIT("BATT: Supply = %s\n", supp->name);
		pr_debug("BATT: Supply = %s\n", supp->name);
		power_supply_changed(supp);
		//printk("BATT: msm_batt_info.current_ps = %d, supp =%d.\n", msm_batt_info.current_ps,supp);
		if(msm_batt_info.current_ps != supp)
		{
	...
	//[ECID:0000]ZTE_BSP maxiaoping 20120830 force to update charger status when power supply changed,end.
	//shiyan added
	first_time = 0 ;
//	pr_info("%s: exit\n", __func__);
       pr_debug("%s: exit\n", __func__);
...
void msm_batt_early_suspend(struct early_suspend *h)
{
	...
	pr_debug("%s: exit\n", __func__);
#endif
//doumingming 20121106
	int rc;

	pr_debug("%s: enter\n", __func__);
       get_monotonic_boottime(&unchg_sleep_time_start);     //doumingming add 20121221
	printk("doumingming:enter early_suspend\n");
...
//doumingming 20121106
	...
	pr_debug("%s: enter\n", __func__);
	batt_cap_resume =1;
//	printk("doumingming:late_resume batt_cap_resume =1\n");
	printk("doumingming:enter late_resume\n");
       get_monotonic_boottime(&unchg_sleep_time_current);     //doumingming add 20121221
	if (msm_batt_info.batt_handle != INVALID_BATT_HANDLE) {
		rc = msm_batt_modify_client(msm_batt_info.batt_handle,
		msm_batt_info.voltage_fail_safe,
		BATTERY_ALL_ACTIVITY,
...
#endif
/*ZTEBSP doumingming add for report info when waked up start*/
void msm_batt_open_rpc(void)
{
      int rc;
...
/*ZTEBSP doumingming add for report info when waked up end*/
...
#endif
/*ZTEBSP doumingming 20120924 modify for new battery end */
...
static  const Batt_deltaTable AC_deltaTable[] = {
   {3300, 30},                        
   {3400, 130},    //shiyan        
   {3600, 120},   
   {3900, 110},    
...
static  const Batt_deltaTable USB_deltaTable[] = {
   {3300, 30},             
   {3400, 108},      
   {3500, 103},                  
   {3600, 102},   
...
u16 demmod_battery_deta_increased_usb_wall(u16 current_voltage, u8 first_time)
{
       u8 step = 0;
       u8 table_count;
       ...
       {
             step = sizeof(USB_deltaTable)/sizeof(Batt_deltaTable);
   		 
             if (current_voltage <= USB_deltaTable[0].voltage)
             {
                 return  USB_deltaTable[0].delta;
             }
             else if (current_voltage >= USB_deltaTable[step-1].voltage)
             {
                 return USB_deltaTable[step-1].delta;
             }
             else
             {    
                 for (table_count = 1; table_count< step; table_count++)
                 {
                     if (current_voltage <= USB_deltaTable[table_count].voltage)
                     {
                        return  USB_deltaTable[table_count-1].delta;
                     }
                 }
             }
       }
       ...
       {
            step = sizeof(AC_deltaTable)/sizeof(Batt_deltaTable);
       	 
            if (current_voltage <= AC_deltaTable[0].voltage)
            {
                return AC_deltaTable[0].delta;
            }
            else if (current_voltage >= AC_deltaTable[step-1].voltage)
            {
                return  AC_deltaTable[step-1].delta;
            }
            else
            {    
                for (table_count = 1; table_count< step; table_count++)
                {
                    if (current_voltage <= AC_deltaTable[table_count].voltage)
                    {
                        return  AC_deltaTable[table_count-1].delta;
                    }
                }
            }
	}
...
u8 vbatt_calculate_batt_capacity(u16 current_voltage)
{
    ...
    const BattFuelCapacity *fuelCapacity;

    fuelCapacity = fuelCapacity_norm;
    
	
    step = sizeof(fuelCapacity_norm)/sizeof(BattFuelCapacity);
    ...
    {    
        for (table_count = 1; table_count< step; table_count++)
        {
            ...
            {
                ret_val = (fuelCapacity[table_count-1].capacity 
                    + ((current_voltage - fuelCapacity[table_count-1].voltage)*(fuelCapacity[table_count].capacity - fuelCapacity[table_count-1].capacity)
...
#endif
//shiyan added 20120529
//doumingming added 20121221
static u32 msm_batt_capacity(u32 current_voltage)
{
	static int first_sample = 1;
	u8 batt_cap;
	static u32 chg_time =0;
	static u32 last_chg_time=0;
	static u32 unchg_time =0;
	static u32 last_unchg_time=0;
//	static u32 chg_final_time =0;
	static u32 unchg_sleep_time = 0;
	u32 battery_volt =0;

	#ifdef BATTERY_DEBUG
	 printk("shiyan:  enter  msm_batt_capacity param voltage =%d mv \n", current_voltage);
	 printk("shiyan:  last_battery_capacity =%d %% \n", last_battery_capacity);
	#endif

	if(current_voltage == 0)
	{
		battery_volt = msm_batt_get_vbatt_voltage();
		if(battery_volt == 0)
		{
			battery_volt =4150;
		}
	...
	if (msm_batt_info.batt_status == POWER_SUPPLY_STATUS_CHARGING)
	{
		...
		last_unchg_time =0;
		if(charge_flag !=1)  {
		ktime_get_ts(&chg_start);
		charge_flag = 1;
		}

		ktime_get_ts(&chg_current);         //doumingming add 20121106
		chg_time = (chg_current.tv_sec - chg_start.tv_sec)/60;    //doumingming add 20121106

		#ifdef BATTERY_DEBUG
		printk("shiyan:  chg_time = %d \n",  chg_time);
		#endif
		...
		else
		{
			if(first_sample)	
			{
				if(current_voltage >= 4150)
				{
					batt_has_charged_full = 1;
					batt_cap = 100;
					last_battery_capacity = 100 ;
					first_sample = 0;
			...
			//if((chg_final_time >= CHG_FINAL_CYCLE_TIME)||((chg_time >= TIMEOUT_MIN)&&(current_voltage > 4100)))
			if((chg_time >= TIMEOUT_MIN)&&(current_voltage > 4100))
			{
				batt_cap = 100 ;
				batt_has_charged_full = 1;
//				printk("shiyan:  chg_final_time =%d min  chg_time=%d min \n",  chg_final_time, chg_time);
			}
			else
			{
			#if 0
				if((current_voltage >= 4180)&&(last_battery_capacity >= 99))
				{
					if(chg_final_time < CHG_FINAL_CYCLE_TIME) //10min
					{
					       ktime_get_ts(chg_final_current);
						chg_final_time = (chg_final_current.tv_sec - chg_final_start)/60;
						#ifdef BATTERY_DEBUG
						printk("shiyan:  chg_final_time = %d \n",  chg_final_time);
						#endif
					}
					batt_cap = 99;
				}
			...
			#endif
				{
					//chg_final_time = 0;	//doumingming 20121106
					
					if(first_sample)	
					{
						battery_volt = battery_volt - demmod_battery_deta_increased_usb_wall(battery_volt, 1);   
					}
					else
					{
						battery_volt = battery_volt - demmod_battery_deta_increased_usb_wall(battery_volt, 0);
					}
					
					batt_cap = vbatt_calculate_batt_capacity(battery_volt);
					#ifdef BATTERY_DEBUG
					printk("shiyan:  after demmod  current_voltage = %d mv\n",  battery_volt);
					printk("shiyan:  cal  battery_capacity  is %d %%\n",  batt_cap);
					#endif
				}
				...
				#else
					if(last_battery_capacity <= 80)
				#endif
					//doumingming 20121017
					{
						if((chg_time >= 3)&&(chg_time != last_chg_time))  {
						batt_cap = last_battery_capacity +1 ;
//						printk("doumingming:  chg_time = %d \n",  chg_time);
						}
						else
						batt_cap = last_battery_capacity ;	
					}
					...
					{
					    if((chg_time % 2 == 0)&&(chg_time != last_chg_time))  {
					        batt_cap = last_battery_capacity +1 ;
//					        printk("doumingming:  chg_time = %d \n",  chg_time);
					        }
					    else
					...
					#if 0
					#ifdef CONFIG_BATTERY_2000mAh
					    batt_cap = initial_battery_capacity + (3*chg_time)/5;
			    		printk("doumingming:initial_battery_capacity %d\n", initial_battery_capacity);
					#else
					    batt_cap = initial_battery_capacity + (2*chg_time)/3;
					#endif
					#endif
				}
			
				if ((batt_cap >= 100)&&(current_voltage >= 4180))
				{
					//batt_cap = 99;
					batt_cap = 100; //doumingming add 20121106
				}
				else if (batt_cap <= 0)
				{
					batt_cap= 1;
				}
				else if ((batt_cap >=100) && (current_voltage < 4180))
				{
				       batt_cap =99;
				}
				#ifdef BATTERY_DEBUG
				 printk("shiyan:  chg final  battery_cap = %d %% \n",  batt_cap);
				#endif
			}
		}
		...
		if(chg_time%10==0)
		{
			pr_debug("shiyan:  every 10 min print battery_cap = %d %% \n",  batt_cap);
		}
...
//		chg_final_time =0;
		if(charge_flag !=2)  {
		ktime_get_ts(&unchg_start);
		charge_flag = 2;
//		initial_battery_capacity = last_battery_capacity;
//		printk("doumingming:initial_battery_capacity %d\n", initial_battery_capacity);
//		printk("doumingming:charge_flag=2 %d\n", charge_flag);
		}
		
		ktime_get_ts(&unchg_current);           //doumingming add 20121106
		unchg_time = (unchg_current.tv_sec - unchg_start.tv_sec)/60;    //doumingming add 20121106
		unchg_sleep_time = (unchg_sleep_time_current.tv_sec - unchg_sleep_time_start.tv_sec)/60;    //doumingming add 20121106
//		printk("doumingming: unchg_sleep_time_current %ld\n", unchg_sleep_time_current.tv_sec);
//		printk("doumingming: unchg_sleep_time_start %ld\n", unchg_sleep_time_start.tv_sec);
//		printk("doumingming: unchg_sleep_time %d\n", unchg_sleep_time);
//		printk("doumingming:unchg_time  %d\n", unchg_time);
//		printk("doumingming:charge_flag=2 %d\n", charge_flag);

		if(first_sample)	
		{
			if(current_voltage >= 4120)
			{
				batt_has_charged_full = 1;
				batt_cap = 100;
				last_battery_capacity = 100 ;
		...
		if(0 == batt_has_charged_full )
		{
			batt_cap = vbatt_calculate_batt_capacity(battery_volt);
			#ifdef BATTERY_DEBUG
			printk("shiyan:  not chg cal  battery_capacity  is %d %%\n",  batt_cap);
			...
			if ((last_battery_capacity != 1000)&&(batt_cap < last_battery_capacity))
			{
				...
				if(batt_cap_resume)
				{
					//batt_cap = last_battery_capacity - 1;
					batt_cap_resume =0;
					batt_cap = (last_battery_capacity*2 + batt_cap)/3  ;
				}
				else
				{
		...
		last_unchg_time = unchg_time;
	}
	
	first_sample = 0;

	 last_battery_capacity = batt_cap ;
...
#ifdef CONFIG_ZTE_PLATFORM//shiyan
