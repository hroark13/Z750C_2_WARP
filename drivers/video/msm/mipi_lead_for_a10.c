...
static  int lcd_gpio_config(void)
{
	...
	gpio_tlmm_config(GPIO_CFG(128, 0, GPIO_CFG_INPUT,
		...
		gpio_tlmm_config(GPIO_CFG(130, 0, GPIO_CFG_INPUT,
		GPIO_CFG_PULL_UP, GPIO_CFG_2MA),GPIO_CFG_ENABLE);
...
/*	switch(panleid & 0xff){
		...
		default:
*/			panleid = lcd_gpio_config();//wangminrong add for lcd a10
				printk("the lcd is nt35510  gpio is %d\r\n",panleid);
					switch(panleid)
					{
...
/* [ECID:0000] ZTEBSP wangbing start 201205281 for lcd, start*/
//[ECID:0000] ZTEBSP wangminrong start 20120629 for  mipi ULPS
if (1) {
	...
	gpio_direction_output(GPIO_LCD_RESET,1);
	msleep(10);
	//[ECID:0000] ZTEBSP wangminrong enter 20120629 for  mipi ULPS
