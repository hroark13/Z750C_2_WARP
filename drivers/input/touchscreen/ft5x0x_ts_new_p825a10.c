...
{
/*[ECID:000000] ZTEBSP wangbing 20120602, for ts */
//#include "Ver0B_20110929_040_9980_app_P740A.i"
//#include "Ver0A_20120602_P825A10_ID0x89_app.i"
#include "Ver0B_20120606_P825A10_ID0x89_app.i"
};
...
unsigned char fts_ctpm_get_upg_vid(void)
{
    unsigned int ui_sz;
    ui_sz = sizeof(CTPM_FW);
    if (ui_sz > 2)
...
unsigned char FTS_CTPM_GetVendorID_Info(void)
{
    ...
    /*********Step 1:Reset  CTPM *****/
    /*write 0xaa to register 0xfc*/
    Fts_i2c_write(update_client,0xfc,0xaa);
    delay_ms(50);
     /*write 0x55 to register 0xfc*/
    Fts_i2c_write(update_client,0xfc,0x55);
    ...
    do
    {
        ...
        i_ret = i2c_master_send(update_client,auc_i2c_write_buf, 2);
    ...
    byte_write(update_client, auc_i2c_write_buf, 4);
    byte_read(update_client,reg_val, 2);
    ...
    byte_write(update_client, auc_i2c_write_buf, 1);
    byte_read(update_client,reg_val, 1);
    ...
    buf[2] = 0x78;
    buf[3] = 0x4;
    byte_write(update_client, buf, 4);
    byte_read(update_client,buf, 1);    
    ...
    i2c_master_send(update_client, auc_i2c_write_buf, 1);
...
int fts_ctpm_auto_upg(const char *buffer,unsigned long count)
{
    unsigned char uc_host_fm_ver=FT5x0x_REG_FW_VER;
    unsigned char uc_tp_fm_ver;
    unsigned char uc_host_fm_vid;
    unsigned char uc_tp_fm_vid;
    int           i_ret;
    unsigned long val;
...
if(val==1)   //upgrade with i file
{
       pbt_buf = CTPM_FW;
	fwsize = sizeof(CTPM_FW);
       uc_host_fm_ver = fts_ctpm_get_upg_ver();
       uc_host_fm_vid = fts_ctpm_get_upg_vid();
 }
 else
 {
    char fwname[15] = "ftsapp.bin";

    //memset(fwname, 0, sizeof(fwname));
    //sprintf(fwname, "%s", buffer);
   // fwname[count-1] = '\0';

    fwsize = ft5x0x_GetFirmwareSize(fwname);
    if(fwsize <= 0)
    {
    ...
    if(ft5x0x_ReadFirmware(fwname, pbt_buf))
    {
    ...
    uc_host_fm_vid = *(pbt_buf+(fwsize-1));
	
  }

    /*FW version/vendorID detect*/
    uc_tp_fm_vid = FTS_CTPM_GetVendorID_Info();
    msleep(500);
    Fts_i2c_read(update_client, FT5x0x_REG_FW_VER, &uc_tp_fm_ver,1) ;//ZTE_TS_XYM_20110830

         printk("[FTS] uc_tp_fm_vid = 0x%x, uc_host_fm_vid = 0x%x\n",
    ...
    i_ret =  fts_ctpm_fw_upgrade(update_client,pbt_buf, fwsize);
       if (i_ret == 0)
        {
            //msleep(300);
            //uc_host_fm_ver = fts_ctpm_get_upg_ver();
    ...
    {
      		if ( uc_host_fm_vid != uc_tp_fm_vid )
      		{
                    printk("[FTS] TP Vendor ID isn't matched.\n");
		
      		}else
      		{
                    printk("[FTS] host FM version is not newer.\n");
      		}
      }	

     if(val!=1)     //upgrade with app file
     {
            kfree(pbt_buf);
      }
...
//fts_ctpm_fw_upgrade(update_client, CTPM_FW, sizeof(CTPM_FW));
    fts_ctpm_auto_upg(buffer,count);
...
//	int j=0, k=0;
	...
	if (ret < 0){
   		printk(KERN_ERR "%s: Fts_i2c_read failed, go to poweroff 2.8 V.\n", __func__);
	    	gpio_direction_output(GPIO_TOUCH_EN_2V8_OUT, 0);
	    	msleep(200);
	    	gpio_direction_output(GPIO_TOUCH_EN_2V8_OUT, 1);
	    	msleep(220);
   		printk(KERN_ERR "%s: Fts_i2c_read failed, go to poweroff. 1.8V\n", __func__);
	    	gpio_direction_output(GPIO_TOUCH_EN_1V8_OUT, 0);
	    	msleep(200);
...
/* ZTEBSP shihuiqin 20120605 for fts +++ */
