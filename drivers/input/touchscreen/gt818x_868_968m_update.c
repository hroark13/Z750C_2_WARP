...
#define HEADER_UPDATE_DATA_GT818X NULL         //      |
...
extern struct i2c_client *guitar_client  ;
...
static u8 gt818x_get_ic_msg(struct i2c_client *client, u16 addr, u8* msg, s32 len)
{
    s32 i = 0;
...
static u8 gt818x_clear_mix_flag(struct i2c_client *client)
{
    s32 i = 0;
    u8 buf[3];
...
static u8 gt818x_get_ic_fw_msg(struct i2c_client *client)
{
    s32 i = 0;
    u8 buf[32];
    struct goodix_ts_data *ts = i2c_get_clientdata(client);
    
    if (fail == gt818x_clear_mix_flag(client))
    {
        return fail;
    }
#if 0
    //Get the mask version in nvram of IC
    if (fail == gt818x_get_ic_msg(client, READ_MSK_VER_ADDR, buf, 1))
    {
    ...
    if (fail == gt818x_get_ic_msg(client, 0x717, buf, 2))
    {
    ...
    if (fail == gt818x_get_ic_msg(client, 0x4011, buf, 1))
    {
    ...
    for (i = 0; i < 2; i++)
    {
        if (fail == gt818x_get_ic_msg(client, 0x694, buf, 1))
        {
            GTP_DEBUG("Get firmware msg in IC error.");
            return fail;
        }
...
u8 gt818x_load_update_file(struct i2c_client *client, st_fw_head* fw_head, u8* data, u8* path)
{
    u8 mask_num = 0;
    int ret = 0;
    int i = 0;
    ...
    {        
		    ...
		    && update_msg.ic_fw_msg.msk_ver[0] == fw_head->msk_ver[0])
        {
            ...
            //The third condition ÅÐ¶ÏÊÇ·ñ¹Ì¼þÉý¼¶
            if (fw_head->version > update_msg.ic_fw_msg.version|| update_msg.ic_fw_msg.version==0xBA
...
u8 gt818x_load_update_header(struct i2c_client *client, st_fw_head* fw_head, u8** data)
{
    const u8* pos;
    int i = 0;
    u8 mask_num = 0;

    pos = HEADER_UPDATE_DATA_GT818X;
...
static u8 gt818x_nvram_store(struct i2c_client *client)
{
    int ret;
    int i;
...
static u8 gt818x_nvram_recall(struct i2c_client *client)
{
    int ret;
...
static u8 gt818x_update_nvram(struct i2c_client *client, st_fw_head* fw_head, u8 *nvram)
{
    int length = 0;
    int ret = 0;
    int write_bytes = 0;
   ...
   // gt818x_clear_mix_flag(ts);
    ...
    while(length < fw_head->lenth)
    {
        ...
        if (ret <= 0)
        {
            ...
            GTP_DEBUG("Write error![gt818x_update_nvram]");
            continue;
        }
        else
        { 
...
static u8 gt818x_update_firmware(struct i2c_client *client, st_fw_head* fw_head, u8 *nvram)
{
    s32 retry;
    s32 i, j;
    s32 ret;
    ...
    gt818x_get_ic_msg(client, 0x1522, buf, 1);
    GTP_DEBUG("IC OSC_CAL:0x%02x.", buf[2]);

    gt818x_clear_mix_flag(client);
    ...
    for (retry = 0; retry < 10; retry++)
    {
        ...
        if (!(status & 0x02))
        {
            if (fail == gt818x_update_nvram(client, fw_head, nvram))
            {
                continue;
            }
            else
...
s32 gt818x_update_proc(void *dir)
{
    ...
    if (!(HEADER_UPDATE_DATA_GT818X != NULL && dir == NULL))
    {
    ...
    if (dir != NULL)
    {
        gt818x_get_ic_fw_msg(guitar_client);
        if(GT818X == ts->chip_type)
        {
            gtp_reset_guitar(10);
        }
    ...
    if (!(HEADER_UPDATE_DATA_GT818X != NULL && dir == NULL))
    {
        if (fail == gt818x_load_update_file(guitar_client, &fw_head, data, (u8*)dir))
        {
    ...
    {
        if (fail == gt818x_load_update_header(guitar_client, &fw_head, &data))
        {
            GTP_ERROR("Load file data failed!");
            goto load_failed;
        }
    ...
    while(retry++ < 5)
    {
        if (fail == gt818x_update_firmware(guitar_client, &fw_head, data))
        {
        ...
        if (fail == gt818x_nvram_store(guitar_client))
        {
        ...
        if (fail == gt818x_get_ic_msg(guitar_client, 0x1201, buf, 1))
        {
        ...
        if (fail == gt818x_nvram_recall(guitar_client))
        {
...
u8 gt818x_init_update_proc(struct goodix_ts_data *ts)
{
    u8 flag = 0;
    ...
    update_msg.fw_flag = gt818x_get_ic_fw_msg(ts->client);
    if (fail == update_msg.fw_flag)
    {
        GTP_DEBUG("Try get ic msg in update mode.");
        for (retry = 0; retry < 5; retry++)
        {
            if (success == gt818x_enter_update_mode(ts->client))
            {
                break;
            }
        }
