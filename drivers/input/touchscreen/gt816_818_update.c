...
static u8 gt818_get_ic_msg(struct i2c_client *client, u16 addr, u8* msg, s32 len)
{
    s32 i = 0;
...
#if 0
static u8 gt818_clear_mix_flag(struct i2c_client *client)
{
    s32 i = 0;
    u8 buf[3];
...
#endif
#if 0
static u8 gt818_get_ic_fw_msg(struct i2c_client *client)
{
    s32 ret = 0;
    s32 i = 0;
    u8 buf[32];
    
    if (fail == gt818_clear_mix_flag(client))
    {
        return fail;
    }

    //Get the mask version in rom of IC
    if (fail == gt818_get_ic_msg(client, READ_MSK_VER_ADDR, buf, 4))
    {
    ...
    for (i = 0; i < 2; i++)
    {
        if (fail == gt818_get_ic_msg(client, READ_FW_MSG_ADDR, buf, 4))
        {
    ...
    if (fail == gt818_get_ic_msg(client, 0x4011, buf, 1))
    {
...
Output:
	Executive outcomes.
*********************************************************/
static s16 gt818_check_version(u16 sw_ver)
{
...
s32 gt818_comfirm_version(struct i2c_client *client)
{
    s32 i = 0;
    s32 count = 0;
    u16 version;
...
u8 gt818_load_update_file(struct i2c_client *client, st_fw_head* fw_head, u8* data, u8* path)
{
    //u8 mask_num = 0;
    u16 checksum = 0;
    int ret = 0;
    ...
    if(gt818_check_version(update_msg.version) != gt818_check_version(update_msg.fw_msg.version))
    {
...
u8 gt818_load_update_header(struct i2c_client *client, st_fw_head* fw_head, u8** data)
{
    u16 checksum = 0;
//    s32 ret = -1;
    s32 i = 0;
...
static u8 gt818_nvram_store(struct i2c_client *client)
{
    int ret;
    int i;
...
static u8 gt818_nvram_recall(struct i2c_client *client)
{
    int ret;
...
static u8 gt818_update_nvram(struct i2c_client *client, st_fw_head* fw_head, u8 *nvram)
{
    int length = 0;
    int ret = 0;
    int write_bytes = 0;
   ...
   // gt818_clear_mix_flag(ts);
    ...
    while(length < fw_head->lenth)
    {
        ...
        if (ret <= 0)
        {
            ...
            GTP_DEBUG("Write error![gt818_update_nvram]");
            continue;
        }
        else
        {
...
static u8 gt818_update_firmware(struct i2c_client *client, st_fw_head* fw_head, u8 *nvram)
{
    s32 retry;
//    s32 i, j;
//    s32 ret;
...
//    u8  tmp[150];
    ...
    gt818_get_ic_msg(client, 0x1522, buf, 1);
    GTP_DEBUG("IC OSC_CAL:0x%02x.", buf[2]);

    gt818_clear_mix_flag(client);
#endif
    for (retry = 0; retry < 10; retry++)
    {
        ...
        if (!(status & 0x02))
        {
            if (fail == gt818_update_nvram(client, fw_head, nvram))
            {
                continue;
            }
            else
    ...
    {
        msleep(1);
        if (fail == gt818_get_ic_msg(client, 0x00ff, buf, 1))
        {
...
s32 gt818_update_proc(void *dir)
{
    ...
    if (dir != NULL)
    {
    //    gt818_get_ic_fw_msg(guitar_client);
    //    gtp_reset_guitar(10);
    }
    else
    {
    ...
    if (!(HEADER_UPDATE_DATA != NULL && dir == NULL))
    {
        if (fail == gt818_load_update_file(guitar_client, &fw_head, data, (u8*)dir))
        {
    ...
    {
        if (fail == gt818_load_update_header(guitar_client, &fw_head, &data))
        {
            GTP_ERROR("Load file data failed!");
            goto load_failed;
        }
    ...
    while(retry++ < 5)
    {
        if (fail == gt818_update_firmware(guitar_client, &fw_head, data))
        {
        ...
        if (fail == gt818_nvram_store(guitar_client))
        {
        ...
        if (fail == gt818_get_ic_msg(guitar_client, 0x1201, buf, 1))
        {
        ...
        if (fail == gt818_nvram_recall(guitar_client))
        {
...
u8 gt818_init_update_proc(struct goodix_ts_data *ts)
{
    struct task_struct *thread = NULL;

    if (fail == gt818_comfirm_version(ts->client))
    {
