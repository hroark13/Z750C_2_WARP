...
static u8 gup_get_ic_fw_msg(struct i2c_client *client)
{
    ...
    /*|-----FLASH-----RAM-----|
      ...
      |------9110-----9110----|
      |------9110P----9111----|*/
    if(!strlen(update_msg.ic_fw_msg.pid))
    {
        if(!memcmp(update_msg.ic_fw_msg.pid, "9111", strlen(update_msg.ic_fw_msg.pid)))
        {
            GTP_DEBUG("IC Mapping Product id:%s", update_msg.ic_fw_msg.pid);
            memcpy(update_msg.ic_fw_msg.pid, "9110P", sizeof(update_msg.ic_fw_msg.pid));
        }
    }
