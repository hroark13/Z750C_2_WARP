...
//[ECID:000000] ZTEBSP zhangjing for cdrom 20120229 start
static struct mass_storage_function_config *mass_storage_config;
//[ECID:000000] ZTEBSP zhangjing for cdrom 20120229 end
...
//[ECID:000000] ZTEBSP zhangjing for 2u disk 20120830 start
	//config->fsg.nluns = 1;
	//config->fsg.luns[0].removable = 1;
     config->fsg.nluns = 2;
	 config->fsg.luns[0].removable = 1;
	 config->fsg.luns[1].removable = 1;
//[ECID:000000] ZTEBSP zhangjing for 2u disk 20120830 end
...
//ztebsp zhangjing add for udisk,20120905,++
err = sysfs_create_link(&f->dev->kobj,
...
// ZTE_MODIFY  dongdeliang, caoyaobin  remove the nmea ,modem, diag ,rndis
#ifdef CONFIG_ZTEUSB_NO_NMDR_PORTS
static struct android_usb_function *supported_functions_close_nmdr_ports[] = {
	&mbim_function,
	&rmnet_smd_function,
	&rmnet_sdio_function,
...
#endif 
// ZTE_MODIFY dongdeliang, caoyaobin end remove the nmea ,modem, diag ,rndis
...
// ZTE_MODIFY caoyaobin do PID check attr
#ifdef CONFIG_ZTEUSB_NO_NMDR_PORTS
/*---------------------------------------------------------------------
    MTP  MTP+ADB  PTP  PTP+ADB  CDROM  CDROM+ADB  MS    MS+ADB MTP+CDROM+ADB
-----------------------------------------------------------------------*/
static char pid_wlist[][4] = { 
       {'0','2','4','4'}, {'0','2','4','5'}, {'0','3','1','1'}, {'0','3','1','0'},
       {'0','3','6','0'}, {'0','4','0','2'}, {'1','3','5','3'}, {'1','3','5','1'},
       {'0','2','9','2'} };

static bool checkpid( const char * buf)
{
	int i=0;
	for (i = 0 ; i < sizeof(pid_wlist)/sizeof(pid_wlist[0]); i++ )
	{
		if(0 == strncmp( buf, pid_wlist[i], sizeof(pid_wlist[0]) ) )
			return 1;
	}
	return 0;
...
#define DESCRIPTOR_PIDWL_ATTR(field, format_string)				\
static ssize_t								\
field ## _show(struct device *dev, struct device_attribute *attr,	\
		char *buf)						\
{									\
...
{									\
	int value;							\
	if (0 == checkpid(buf))return -EINVAL;				\
	if (sscanf(buf, format_string, &value) == 1) {			\
		device_desc.field = value;				\
		return size;						\
	}								\
...
static DEVICE_ATTR(field, S_IRUGO | S_IWUSR, field ## _show, field ## _store);
#endif
// ZTE_MODIFY end  caoyaobin do PID check attr 


DESCRIPTOR_ATTR(idVendor, "%04x\n")
#ifdef CONFIG_ZTEUSB_NO_NMDR_PORTS
DESCRIPTOR_PIDWL_ATTR(idProduct, "%04x\n")
#else
...
static int __init init(void)
{
	...
	#ifdef CONFIG_ZTEUSB_NO_NMDR_PORTS
	dev->functions = supported_functions_close_nmdr_ports;
	#endif
	INIT_LIST_HEAD(&dev->enabled_functions);
