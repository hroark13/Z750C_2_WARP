...
static struct msm_camera_i2c_reg_conf ov7692_recommend_settings[] = {
	...
	{0xb5, 0x40},
	{0xff, 0x00},
#if (defined CONFIG_PROJECT_P825F01) || (defined CONFIG_PROJECT_P865F01) || (defined CONFIG_PROJECT_P865V30) || (defined CONFIG_PROJECT_P825B20) ||defined ( CONFIG_PROJECT_P865V20)
	{0x0c, 0xc0},//zhangqi add for P825F01
#else
	{0x0c, 0x00},
#endif
	{0x62, 0x10},
    ...
    {0xa1, 0xb0},                  
    {0xa2, 0x0f}, 
    {0x31, 0x83}, //0x87 for 15fps ; 0x83 for 30fps
    {0xd2, 0x07},  //enable saturation       
    {0xd8, 0x50}, //saturation                 
    {0xd9, 0x50},
    {0x15, 0xa8},
};
...
static struct msm_camera_i2c_reg_conf ov7692_brightness[][2] = {
	{  {0xd3, 0x48},{0xdc, 0x09},}, /*EXPOSURECOMPENSATIONN2*/
	{  {0xd3, 0x28},{0xdc, 0x09},}, /*EXPOSURECOMPENSATIONN2*/
	{  {0xd3, 0x10},{0xdc, 0x01},}, /*EXPOSURECOMPENSATIONN2*/
	{  {0xd3, 0x20},{0xdc, 0x01},}, /*EXPOSURECOMPENSATIONN2*/
	{  {0xd3, 0x40},{0xdc, 0x01},}, /*EXPOSURECOMPENSATIONN2*/
};
static struct msm_camera_i2c_conf_array ov7692_brightness_confs[][1] = {
	{{ov7692_brightness[0], ARRAY_SIZE(ov7692_brightness[0]), 0,
		MSM_CAMERA_I2C_SET_BYTE_WRITE_MASK_DATA},},
	{{ov7692_brightness[1], ARRAY_SIZE(ov7692_brightness[1]), 0,
		MSM_CAMERA_I2C_SET_BYTE_WRITE_MASK_DATA},},
	{{ov7692_brightness[2], ARRAY_SIZE(ov7692_brightness[2]), 0,
		MSM_CAMERA_I2C_SET_BYTE_WRITE_MASK_DATA},},
	{{ov7692_brightness[3], ARRAY_SIZE(ov7692_brightness[3]), 0,
		MSM_CAMERA_I2C_SET_BYTE_WRITE_MASK_DATA},},
	{{ov7692_brightness[4], ARRAY_SIZE(ov7692_brightness[4]), 0,
		MSM_CAMERA_I2C_SET_BYTE_WRITE_MASK_DATA},},
};
...
#if defined (CONFIG_PROJECT_P825F01) || defined (CONFIG_PROJECT_P865F01)  ||defined (CONFIG_PROJECT_P825A20)|| defined (CONFIG_PROJECT_P865V30) ||defined ( CONFIG_PROJECT_P865V20)
	.sensor_id = 0x76,
#else
	.sensor_id = 0x7692,
#endif
...
static void ov7692_release_gpio(void)
{
	gpio_free(ov7692_pwdn_gpio);
}
...
static int ov7692_probe_init_gpio(const struct msm_camera_sensor_info *data)
{
	...
	rc = gpio_request(ov7692_pwdn_gpio, "ov7692");
	if (rc < 0)
		{
		pr_err("%s: gpio_request---ov7692_pwdn_gpio failed!",
			 __func__);
		return rc;
		}

	rc = gpio_tlmm_config(GPIO_CFG(ov7692_pwdn_gpio, 0,
	...
	gpio_direction_output(ov7692_pwdn_gpio, 1);
...
{
	CDBG("%s\n", __func__);
	gpio_set_value(ov7692_pwdn_gpio, 0);//power up
		...
		const struct i2c_device_id *id)
{
	...
	if (s_info == NULL) {
      ...
      rc = ov7692_probe_init_gpio(s_info);
      if(rc < 0)
      	{
...
//[ECID0000000] zhangqi add for double cam for P825F01 begin 2012.7.23
#if (defined CONFIG_PROJECT_P825F01) || (defined CONFIG_PROJECT_P865F01)  ||(defined CONFIG_PROJECT_P825A20)|| (defined CONFIG_PROJECT_P865V30) ||defined ( CONFIG_PROJECT_P865V20)
	.probe= ov7692_sensor_i2c_probe,
#else
	.probe  = msm_sensor_i2c_probe,
#endif
//[ECID0000000] zhangqi add for double cam for P825F01 end 2012.7.23
	.driver = {
		.name = SENSOR_NAME,
	},
};
...
/* zhangzhao fix front sensor currnt problem*/
       rc = msm_camera_i2c_write(s_ctrl->sensor_i2c_client,0xff,0x01,1);
	if (rc < 0) {
		pr_err("%s: i2c write  failed   %d\n", __func__,__LINE__);
	}
       rc = msm_camera_i2c_write(s_ctrl->sensor_i2c_client,0xb4,0xc0,1);
	if (rc < 0) {
		pr_err("%s: i2c write  failed   %d\n", __func__,__LINE__);
	}
       rc = msm_camera_i2c_write(s_ctrl->sensor_i2c_client,0xb5,0x40,1);
	if (rc < 0) {
		pr_err("%s: i2c write  failed   %d\n", __func__,__LINE__);
	}
       rc = msm_camera_i2c_write(s_ctrl->sensor_i2c_client,0xff,0x00,1);
	if (rc < 0) {
		pr_err("%s: i2c write  failed   %d\n", __func__,__LINE__);
	}
	return rc;
...
/* zhangzhao fix front sensor currnt problem*/
	...
	if (rc < 0) {
       ...
       rc = msm_camera_i2c_write(s_ctrl->sensor_i2c_client,0xb4,0x40,1);
	if (rc < 0) {
		pr_err("%s: i2c write  failed   %d\n", __func__,__LINE__);
	}
       rc = msm_camera_i2c_write(s_ctrl->sensor_i2c_client,0xb5,0x30,1);
	if (rc < 0) {
		pr_err("%s: i2c write  failed   %d\n", __func__,__LINE__);
	}
       rc = msm_camera_i2c_write(s_ctrl->sensor_i2c_client,0xff,0x00,1);
