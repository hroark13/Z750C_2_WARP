...
#include <linux/gpio.h>         //doumingmign add20121206
...
//doumingming add 20121206 ++
      ...
      printk("doumingming: enter hsusb_chg_connected");
	...
	if (chgtype == USB_CHG_TYPE__WALLCHARGER) {
     // gpio_request(ZTE_GPIO_CHARGE_CURRENT, "ZTE_GPIO_CHARGE_CURRENT");
      gpio_direction_output(ZTE_GPIO_CHARGE_CURRENT, 1);
            printk("doumingming:hsusb_chg_connected max8903_chg_ac_type");
	}
	else {
      gpio_direction_output(ZTE_GPIO_CHARGE_CURRENT, 0);
	}
#endif
//doumingming add 20121206 --
