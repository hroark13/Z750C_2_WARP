...
#define GPIO_SKU1_CAM_VGA_SHDN    18 //zhangzhao 18
#define GPIO_SKU3_CAM_5MP_SHDN_N   122 //zhangzhao 5         /* PWDN */
#define GPIO_SKU3_CAM_5MP_CAMIF_RESET   123 //zhangzhao 6    /* (board_is(EVT))?123:121 RESET */

#define GPIO_CAMERA_2V8_AVDD_EN 35 /* SKU1&SKU3 2.8V LDO */
#define GPIO_CAMERA_2V8_MOTO_EN 116 /* SKU1&SKU3 2.8V LDO */
...
static void evb_camera_gpio_cfg(void)
{
	...
	//zhangzhao modified for sensor gpio
	//NOTE here for the gpio that share by the front and back camera,
	// appropriative gpio should be requested in its sesnor driver
}
...
static void camera_power_init(void)
{
	...
	/* LDO_EXT2V8 */
	rc = gpio_request(GPIO_CAMERA_2V8_AVDD_EN, "camavdd");
	if (rc < 0)
		pr_err("%s: gpio_request GPIO_CAMERA_2V8_AVDD_EN failed!",
			 __func__);
	pr_err("gpio_tlmm_config GPIO_CAMERA_2V8_AVDD_EN %d\r\n", GPIO_CAMERA_2V8_AVDD_EN);

/*[ECID:000000] ZTEBSP wangbing, for P825A10 camera, 20120528 */
#if 0
	rc = gpio_tlmm_config(GPIO_CFG(GPIO_CAMERA_2V8_AVDD_EN, 0,
	...
	rc = gpio_tlmm_config(GPIO_CFG(GPIO_CAMERA_2V8_AVDD_EN, 0,
		...
		GPIO_CFG_2MA), GPIO_CFG_ENABLE);
	gpio_direction_output(GPIO_CAMERA_2V8_AVDD_EN, 1);
#endif
	if (rc < 0) {
		pr_err("%s:unable to enable GPIO_CAMERA_2V8_AVDD_EN!\n",
			 __func__);
		gpio_free(GPIO_CAMERA_2V8_AVDD_EN);
	}


	rc = gpio_request(GPIO_CAMERA_1V8_EN, "camdvdd");
	if (rc < 0)
		pr_err("%s: gpio_request GPIO_CAMERA_1V8_EN failed!",
			 __func__);

	pr_err("gpio_tlmm_config GPIO_CAMERA_1V8_EN%d\r\n", GPIO_CAMERA_1V8_EN);
	rc = gpio_tlmm_config(GPIO_CFG(GPIO_CAMERA_1V8_EN, 0,
		GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN,
		GPIO_CFG_2MA), GPIO_CFG_ENABLE);
	if (rc < 0) {
		pr_err("%s:unable to enable GPIO_CAMERA_1V8_EN!\n",
			 __func__);
		gpio_free(GPIO_CAMERA_1V8_EN);
	}

	gpio_direction_output(GPIO_CAMERA_1V8_EN, 1);

	rc = gpio_request(GPIO_CAMERA_2V8_MOTO_EN, "cammoto");
	if (rc < 0)
		pr_err("%s: gpio_request GPIO_CAMERA_2V8_MOTO_EN failed!",
			 __func__);

	pr_err("gpio_tlmm_config GPIO_CAMERA_2V8_MOTO_EN %d\r\n", GPIO_CAMERA_2V8_MOTO_EN);
	rc = gpio_tlmm_config(GPIO_CFG(GPIO_CAMERA_2V8_MOTO_EN, 0,
		GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN,
		GPIO_CFG_2MA), GPIO_CFG_ENABLE);
	if (rc < 0) {
		pr_err("%s:unable to enable GPIO_CAMERA_2V8_MOTO_EN!\n",
			 __func__);
		gpio_free(GPIO_CAMERA_2V8_MOTO_EN);
	}
/*[ECID:000000] ZTEBSP wangbing, for P825A10 camera, 20120528 */
